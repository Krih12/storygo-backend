...
const seriesController = {
  ...
  getAllSeries: async (req, res) => {
    try {
      ...
      const countResult = await query(`SELECT COUNT(*) FROM series s ${whereClause}`, params);
      ...
      return res.status(500).json({ status: 'error', message: 'Error fetching series' });
    }
  },
  ...
  createSeries: async (req, res) => {
    ...
    try {
      ...
      let thumbnail_url = req.file?.path || null;
      ...
      return res.status(500).json({ status: 'error', message: 'Error creating series' });
    } finally {
      client.release();
    }
  },
  ...
  updateSeries: async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ status: 'error', message: 'Error updating series' });
    } finally {
      client.release();
    }
  },
  ...
  deleteSeries: async (req, res) => {
    ...
    try {
      ...
      await client.query('COMMIT');
      return res.json({ status: 'success', message: 'Series deleted successfully' });
    } catch (error) {
      await client.query('ROLLBACK');
      console.error('Delete series error:', error);
      return res.status(500).json({ status: 'error', message: 'Error deleting series' });
    } finally {
      client.release();
    }
  }
